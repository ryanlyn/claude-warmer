...
describe('SessionRow', () => {
  ...
  it('shows warm badge for warm sessions', () => {
    const { lastFrame } = render(<SessionRow session={makeSession({ isWarm: true })} highlighted={false} layout={layout} warmingActive={false} />);
    expect(lastFrame()!).toContain('[w]');
  });

  it('shows cold badge for cold sessions', () => {
    const { lastFrame } = render(<SessionRow session={makeSession({ isWarm: false, isLive: false })} highlighted={false} layout={layout} warmingActive={false} />);
    expect(lastFrame()!).toContain('[c]');
  });

  it('shows live indicator and warm badge for live sessions', () => {
    const { lastFrame } = render(<SessionRow session={makeSession({ isLive: true, isWarm: true })} highlighted={false} layout={layout} warmingActive={false} />);
  ...
  it('does not show live indicator for non-live sessions', () => {
    const { lastFrame } = render(<SessionRow session={makeSession({ isLive: false, isWarm: true })} highlighted={false} layout={layout} warmingActive={false} />);
    const frame = lastFrame()!;
  ...
  it('shows model short name at wide widths', () => {
    ...
    const { lastFrame } = render(<SessionRow session={makeSession()} highlighted={false} layout={wide} warmingActive={false} />);
  ...
  it('shows expiry cost for warm sessions at wide widths', () => {
    const wide = computeLayout(140);
    const { lastFrame } = render(<SessionRow session={makeSession({ isWarm: true })} highlighted={false} layout={wide} warmingActive={false} />);
  ...
  it('shows expiry dash for cold sessions at wide widths', () => {
    ...
    const { lastFrame } = render(<SessionRow session={session} highlighted={false} layout={wide} warmingActive={false} />);
    const frame = lastFrame()!;
    expect(frame).toContain('-');
  });
  ...
  it('shows expiry cost for live warm sessions at wide widths', () => {
    const wide = computeLayout(140);
    const { lastFrame } = render(<SessionRow session={makeSession({ isLive: true, isWarm: true })} highlighted={false} layout={wide} warmingActive={false} />);
    expect(lastFrame()!).toContain('$1.05');
  });
  ...
  it('shows dash for warm cost on unselected sessions', () => {
    ...
    const { lastFrame } = render(<SessionRow session={session} highlighted={false} layout={layout} warmingActive={false} />);
    const frame = lastFrame()!;
    expect(frame).toContain('-');
  });

  it('shows warm count', () => {
    const { lastFrame } = render(<SessionRow session={makeSession({ warmCount: 5 })} highlighted={false} layout={layout} warmingActive={false} />);
    expect(lastFrame()!).toContain('5');
  });

  it('shows cwd directory name when visible', () => {
    const wide = computeLayout(100);
    const { lastFrame } = render(<SessionRow session={makeSession({ cwd: '/Users/ryan/dev/my-project' })} highlighted={false} layout={wide} warmingActive={false} />);
    expect(lastFrame()!).toContain('my-project');
  });

  it('handles trailing slash in cwd', () => {
    const wide = computeLayout(100);
    const { lastFrame } = render(<SessionRow session={makeSession({ cwd: '/Users/ryan/dev/project/' })} highlighted={false} layout={wide} warmingActive={false} />);
    expect(lastFrame()!).toContain('project');
  });

  it('truncates long cwd names', () => {
    const wide = computeLayout(100);
    const { lastFrame } = render(<SessionRow session={makeSession({ cwd: '/Users/ryan/dev/very-long-directory-name-here' })} highlighted={false} layout={wide} warmingActive={false} />);
    const frame = lastFrame()!;
    expect(frame).toContain('~');
  });

  it('shows dash for next warm when not scheduled', () => {
    const { lastFrame } = render(<SessionRow session={makeSession({ nextWarmAt: null })} highlighted={false} layout={layout} warmingActive={false} />);
  ...
  it('shows green badge when warming is active and session is selected and warm', () => {
    ...
    const { lastFrame } = render(<SessionRow session={session} highlighted={false} layout={layout} warmingActive={true} />);
    const frame = lastFrame()!;
    expect(frame).toContain('[w]');
  });
